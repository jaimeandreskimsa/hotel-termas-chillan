...
export default function LaGrietaPage() {
  ...
  useEffect(() => {
    fetch("/api/restaurantes/lagrieta").then(r => r.json()).then(d => {
  ...
  return (
    <div className="min-h-svh bg-[#FFFBF3]">
      ...
      <div>
        ...
        {schedules.length > 0 && (
          <div className="px-4 py-3 bg-[#F3EDE4] border-b border-[#EDE6D8] text-center">
        ...
        <div className="px-4 py-5 pb-24 md:pb-12 flex flex-col gap-4 md:max-w-3xl md:mx-auto">
          {(() => {
            ...
            return Object.entries(bySubcategory).map(([sub, subItems]) => (
              <div key={sub}>
                <h3 className="font-playfair font-bold text-[#54432B] text-[32px] leading-none text-center mb-3">{sub}</h3>
                <div className="flex flex-col gap-2">
                  {subItems.map(item => (
                    <div key={item.id} className="bg-[#F3EDE4] rounded-2xl p-4 border border-[#EDE6D8] shadow-sm">
                      <div className="flex justify-between items-start gap-2">
                        <h3 className="text-[#3D2B1F] text-[14px] font-medium flex-1">{item.name}</h3>
                        {item.price && <span className="shrink-0" style={{ fontFamily: 'Cooper Hewitt, sans-serif', fontWeight: 705, fontSize: 15, lineHeight: 1, color: '#54432B' }}>{item.price}</span>}
                      </div>
